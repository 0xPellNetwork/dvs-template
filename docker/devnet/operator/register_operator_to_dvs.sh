...
function load_defaults {
  ...
  export OPERATOR_PUBLIC_RPC_URL=${OPERATOR_PUBLIC_RPC_URL:operator:12345}
...
function init_pelldvs_config {
  ...
  PELL_DELEGATION_MANAGER=$(fetch_pell_address "delegation_manager_proxy")
  ...
  if [ -z "$PELL_DELEGATION_MANAGER" ]; then
    echo "PELL_DELEGATION_MANAGER is not set"
    exit 1
  fi
  ...
  update-config pell_delegation_manager_address "$PELL_DELEGATION_MANAGER"
